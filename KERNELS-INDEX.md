...
`open_ended_evolve`, `lightrag_compare`, `cognee_compare`, `graph_stable`. **ALL Phase-6 kernels wired
(2026-08-14).** Phase 6 complete: the previously-orphaned kernels are now USED in live paths (validator
stack onto the DAG, flywheels into the organism, retrieval into the read plane, organisms routed through
patala's scheduler).
