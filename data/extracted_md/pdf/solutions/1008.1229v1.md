...
system‟s statistical entropy show that molecular interactions do not destroy single-
...
contain a finite quantity of information; its support must have finite phase-space volume,
                                           2
which in general will greatly exceed           , the limit set by Heisenberg‟s uncertainty
...
where
     ...
     ( )
O           is the value of the macroscopic variable O in the macrostate . We discuss the
...
We interpret the density operator  ( ) in Eq. (10) as referring to such an ensemble:
    ...
    Interpret the quantity Tr ( ( )O) as the average value of the macroscopic variable
        ( )
    O          in a cosmological ensemble.                                                          (14)

                   ( )
As in (10), O             is the value of O , the macroscopic counterpart of the observable O, in
...
Like the earlier rule, it treats measurement as a primitive concept. In contrast, rules (10)
                                                                        ( )
and (14) do not mention measurement; it is trivially true that O               is the result of
...
characterized by a probability distribution { pi } of quantum states i M . Assume that the

initial state of the measured system S is a superposition, with coefficients c k , of

eigenstates k S of an observable K. As in von Neumann‟s account, a measurement of K

produces an entangled quantum state of the combined system:

                               
                     ck k S  i M   ck k S j(k,i) M   ck k, j(k,i)                (19)
                      k                 k                   k
...
about the relative phases of quantum states j(k,i) M , j( k ,i) M belonging to different
