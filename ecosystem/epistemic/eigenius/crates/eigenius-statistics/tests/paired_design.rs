...
//! D52 Phase 2 — Paired-design dispatch end-to-end.
//!
//! Exercises the Phase 2 `stats:Paired` smart constructor + the new
//! Paired verifier arm. Confirms:
//!  - The smart constructor lands at product position
//!    `(CompleteRandom, PairedBlocking, SingleFactor, _, CrossSectional)`
//!  - The verifier chunks interleaved `[b0, a0, b1, a1, …]`
//!    observations into pairs and runs the paired t-test
//!  - A pre/post-treatment fixture with a consistent within-pair drop
//!    Holds at α = 0.05 even though the same data treated as IID
//!    two-sample would be much weaker (the false-positive avoidance
//!    motivation behind D52 §5.1)
...
fn build_paired_chain() -> ExecutionContext {
    ...
    let fixture_source = include_str!("fixtures/paired_design.esl");
    let fixture_resources = esl::compile_against_layer(fixture_source, &stats_layer)
        .unwrap_or_else(|errs| {
            panic!(
                "paired_design.esl failed to compile: {}",
                errs.into_iter()
                    .map(|e| format!("{e:?}"))
                    .collect::<Vec<_>>()
                    .join("; ")
    ...
    let mut fixture_builder = LayerBuilder::new("paired-fixture", Some(stats_layer));
...
fn paired_design_recomputes_to_holds() {
    ...
    let claim_iri =
        Iri::parse("urn:eigenius:demo:bp:claim_treatment_lowers_bp").expect("claim IRI");
    let claim_arc = ctx
        .resolve(&claim_iri)
    ...
    // Paired t on 5 pre/post pairs with mean diff = 7, sd ≈ 2.12
    // gives t ≈ 7.4, df = 4, p ≈ 0.002 — clear Holds at α = 0.05.
    assert_eq!(
        ctor,
        wk::VERDICT_HOLDS,
        "expected Holds — paired t-test on consistent pre/post drops should reject H0; \
    ...
    assert!(
        p_value < 0.01,
        "paired test on these pairs should give p ≪ 0.01; got p = {p_value}"
    );
    ...
    assert!(
        t_stat.abs() > 5.0,
        "|t| should be well above 5 for these consistent pre/post drops; got t = {t_stat}"
    );
}

/// Same raw numbers, but if a misguided author treated them as IID
/// two-sample (10 patients in two groups rather than 5 patients
/// matched), the test would be much weaker — the paired structure is
/// what powers the rejection. This test confirms the smart-constructor
/// dispatch picks the right product position automatically: there is
/// no way for the author to accidentally route through the IID arm by
/// passing the same pairs to `stats:IID` because the value shape is
/// different (a flat interleaved float array vs two separate group
/// arrays).
#[test]
fn paired_data_routes_only_through_paired_dispatch() {
    let ctx = build_paired_chain();
    let claim_iri =
        Iri::parse("urn:eigenius:demo:bp:claim_treatment_lowers_bp").expect("claim IRI");
    let claim_arc = ctx
    ...
    let blocking_ctor = bundle_json["args"][1]["ctor"]
        .as_str()
        .expect("blocking arg has ctor");
    assert_eq!(
        blocking_ctor, "PairedBlocking",
        "stats:Paired must land at the PairedBlocking position; got blocking = {blocking_ctor}"
    );
}
