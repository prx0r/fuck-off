            ...
            loop {
                ...
                return match find_trailers(&buf[..])? {
                    ...
                    FindTrailers::Done(len) => Poll::Ready(match len {
                        0 => {
                            // Bug fix: flush any trailers parsed earlier but
                            // not yet emitted (happens when a single hyper
                            // Data frame contained both the data and trailer
                            // frames; the Trailer(N>0) branch emitted Frame::data
                            // and stored trailers, but then end-of-stream
