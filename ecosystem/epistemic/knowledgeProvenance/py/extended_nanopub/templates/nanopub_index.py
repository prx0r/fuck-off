...
from extended_nanopub.definitions import DUMMY_NAMESPACE, DUMMY_URI, MAX_NP_PER_INDEX
from extended_nanopub.namespaces import NPX, PAV
...
class NanopubIndex(ExtendedNanopub):
    """Publish a list of extended_nanopub URIs in a Nanopub Index

    Args:
        conf: config for the extended_nanopub
        np_list: List of extended_nanopub URIs
    ...
    def __init__(
        self,
        conf: NanopubConf,
        np_list: Union[List[str], List[ExtendedNanopub]],
        title: str,
        description: str,
        creation_time: str,
        creators: List[str],
        ...
        for np in np_list:
            if isinstance(np, ExtendedNanopub):
