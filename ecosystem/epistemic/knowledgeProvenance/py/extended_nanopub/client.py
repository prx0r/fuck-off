"""This module includes a client for the extended_nanopub server.
"""
...
class NanopubClient:
    ...
    Args:
        use_test_server (bool): Toggle using the test extended_nanopub server.
        use_server (str): Provide the URL of a extended_nanopub server to use
    """
    ...
    def __init__(
        ...
        if use_test_server:
            ...
            if use_server not in NANOPUB_SERVER_LIST:
                log.warn(f"{use_server} is not in our list of extended_nanopub servers. {', '.join(NANOPUB_SERVER_LIST)}\nMake sure you are using an existing Nanopub server.")
    ...
    def find_nanopubs_with_text(
        ...
        Search the extended_nanopub servers for any nanopubs matching the
        given search text.
    ...
    def find_nanopubs_with_pattern(
        ...
        Search the extended_nanopub servers for any nanopubs matching the given RDF pattern. You can leave
    ...
    def find_retractions_of(
        self, source: Union[str, ExtendedNanopub], valid_only=True
    ) -> List[str]:
        ...
        Args:
            source (str or extended_nanopub.Publication): URI or Nanopub object to find retractions for
        ...
        if isinstance(source, ExtendedNanopub):
        ...
        if valid_only:
            source_publication = ExtendedNanopub(
                source_uri=uri,
    ...
    def _query_grlc(params: dict, endpoint: str, grlc_url: str) -> requests.Response:
        """Query a specific extended_nanopub server grlc endpoint."""
    ...
    def _query_grlc_try_servers(
        ...
        """Query the extended_nanopub server grlc endpoint.

        Query a extended_nanopub grlc server endpoint (for example: find_text). Try several of the extended_nanopub
        garlic servers.
        ...
        raise requests.HTTPError(
            f"Could not get response from any of the extended_nanopub grlc "
            f"endpoints.{resp}"
        )
    ...
    def _search(self, endpoint: str, params: dict):
        """
        General extended_nanopub server search method. User should use e.g. find_nanopubs_with_text,
        find_things etc.
    ...
    def _parse_search_result(result: dict):
        """
        Parse a extended_nanopub search result (i.e. referring to one matching nanopublication).
