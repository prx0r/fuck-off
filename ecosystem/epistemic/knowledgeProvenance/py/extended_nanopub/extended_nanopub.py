...
from extended_nanopub.definitions import MAX_TRIPLES_PER_NANOPUB, NANOPUB_FETCH_FORMAT, NANOPUB_TEST_SERVER
from extended_nanopub.namespaces import HYCL, NP, NPX, NTEMPLATE, ORCID, PAV
from extended_nanopub.nanopub_conf import NanopubConf
from extended_nanopub.profile import ProfileError
from extended_nanopub.sign_utils import add_signature, publish_graph, verify_signature, verify_trusty
from extended_nanopub.utils import MalformedNanopubError, NanopubMetadata, extract_enp_metadata, log


class ExtendedNanopub:
    """A Nanopub object, containing: the RDF that defines the nanopublication;
    configuration for formatting and publishing the extended_nanopub; functions for validating, signing, publishing

    Attributes:
        config (NanopubConfig): Config for the extended_nanopub
    ...
    def __init__(
        ...
        # Get the extended_nanopub RDF depending on how it is provided:
        # source URI, rdflib graph, or file
        if source_uri:
            # If source URI provided we retrieve the extended_nanopub from the servers
            ...
            self._metadata = extract_enp_metadata(self._rdf)
        else:
        ...
        self._knowledgeprov = Graph(self._rdf.store, self._metadata.knowledgeprov)

        self._assertion += assertion
        self._provenance += provenance
        self._pubinfo += pubinfo
        self._knowledgeprov += knowledgeprov

        # Concatenate prefixes declarations from all provided graphs in the main graph
        for user_rdf in [assertion, provenance, pubinfo, knowledgeprov]:
            if user_rdf is not None:
                for prefix, namespace in user_rdf.namespaces():
                    self._rdf.bind(prefix, namespace)

        # Add Head graph if the extended_nanopub was not provided as trig/nquads
        if not rdf and not source_uri:
            ...
            self._head.add((
                self._metadata.namespace[""],
                NP.hasKnowledgeProv,
                self._knowledgeprov.identifier,
            ))

        # Add triples to the extended_nanopub depending on the provided NanopuConf (e.g. creator, date)
    ...
    def update_from_signed(self, signed_g: ConjunctiveGraph) -> None:
        ...
        self._metadata = extract_enp_metadata(signed_g)
    ...
    def sign(self) -> None:
        ...
        if not self._conf.profile:
            raise ProfileError("Profile not available, cannot sign the extended_nanopub")
        if self._metadata.signature:
            raise MalformedNanopubError(f"The extended_nanopub have already been signed: {self.source_uri}")
        ...
        if self.is_valid:
            ...
            raise MalformedNanopubError("The extended_nanopub is not valid, cannot sign it")
    ...
    def publish(self) -> None:
        ...
        if self._introduces_concept:
            ...
            # published with a URI that looks like [published extended_nanopub URI]#step.
    ...
    def is_valid(self) -> bool:
        ...
        np_meta = extract_enp_metadata(self._rdf)
        ...
        if not found_pubinfo:
            raise MalformedNanopubError(f"The pubinfo graph should contain at least one triple that has the extended_nanopub URI as subject: \033[1m{np_uri}\033[0m")

        # TODO: add more checks for trusty and signature
        # if self._metadata.signature:
        #     if self.has_valid_signature is False:
        #         raise MalformedNanopubError("The extended_nanopub is not valid")
        return True
    ...
    @property
    def knowledgeprov(self):
        return self._knowledgeprov
    ...
    def signed_with_public_key(self) -> Optional[str]:
        np_sig = extract_enp_metadata(self._rdf)
    ...
    def _handle_publication_attributed_to(
        ...
        if attribute_publication_to_profile:
            if not self._profile:
                raise MalformedNanopubError("No extended_nanopub profile provided, but attribute_publication_to_profile is enabled")
    ...
    #       For example, if the extended_nanopub's URI is www.purl.org/ABC123 then the blank node will be
    ...
    #     nanopublication's URI), but it should still lie within the space of the extended_nanopub.
    #     Furthermore, the URI the extended_nanopub is published to is not known ahead of time.
    #     """
    #     for s, p, o in rdf:
