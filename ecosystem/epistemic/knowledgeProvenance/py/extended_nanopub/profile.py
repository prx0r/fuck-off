"""
This module holds objects and functions to load a extended_nanopub user profile.
"""
...
from extended_nanopub.utils import log

PROFILE_INSTRUCTIONS_MESSAGE = '''
    Follow these instructions to correctly setup your extended_nanopub profile:
...
class Profile:
    ...
    Attributes:
        ...
        introduction_nanopub_uri (Optional[str]): URI of the user's profile extended_nanopub
    """
    ...
    def __init__(
        ...
        if not private_key:
            ...
            try:
                ...
                raise ProfileError(
                    f'Private key file {private_key} for extended_nanopub not found.\n'
                    f'Maybe your extended_nanopub profile was not set up yet or not set up '
                    f'correctly. \n{PROFILE_INSTRUCTIONS_MESSAGE}'
                )
        else:
        ...
        if not public_key:
            ...
            if isinstance(public_key, Path):
                try:
                    ...
                    raise ProfileError(
                        f'Private key file {public_key} for extended_nanopub not found.\n'
                        f'Maybe your extended_nanopub profile was not set up yet or not set up '
                        f'correctly. \n{PROFILE_INSTRUCTIONS_MESSAGE}'
                    )
            else:
    ...
    def generate_keys(self) -> str:
        ...
        # Format private and public keys to remove header/footer and all newlines, as this is required by extended_nanopub-java
    ...
    def store(self, folder: Path = USER_CONFIG_DIR) -> Path:
        """Stores the extended_nanopub user profile. By default the profile is stored in `HOME_DIR/.extended_nanopub/profile.yaml`.
...
def load_profile(profile_path: Union[Path, str] = DEFAULT_PROFILE_PATH) -> Profile:
    """Retrieve extended_nanopub user profile.

    By default the profile is stored in `HOME_DIR/.extended_nanopub/profile.yaml`.
    ...
    try:
        ...
        msg = (f'{e}\nYour extended_nanopub profile has not been set up yet, or is not set up correctly.\n'
