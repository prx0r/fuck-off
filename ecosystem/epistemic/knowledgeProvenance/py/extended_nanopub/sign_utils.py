...
from extended_nanopub.definitions import NANOPUB_SERVER_LIST, NP_PURL, NP_TEMP_PREFIX
from extended_nanopub.namespaces import NPX
from extended_nanopub.profile import Profile
from extended_nanopub.trustyuri.rdf import RdfHasher, RdfUtils
from extended_nanopub.trustyuri.rdf.RdfPreprocessor import transform
from extended_nanopub.utils import MalformedNanopubError, extract_enp_metadata, log


def add_signature(g: ConjunctiveGraph, profile: Profile, dummy_namespace: Namespace, pubinfo_uri: URIRef) -> ConjunctiveGraph:
    """Implementation in python of the process to sign a extended_nanopub with a RSA private key"""
    g.add((
        dummy_namespace["sig"],
        NPX["hasPublicKey"],
...
def replace_trusty_in_graph(trusty_artefact: str, dummy_ns: str, graph: ConjunctiveGraph):
    ...
    if str(dummy_ns).startswith(NP_TEMP_PREFIX):
        ...
        # prefix is used in the dummy extended_nanopub URI
...
def publish_graph(g: ConjunctiveGraph, use_server: str = NANOPUB_SERVER_LIST[0]) -> bool:
    """Publish a signed extended_nanopub to the given extended_nanopub server.
    """
    log.info(f"Publishing to the extended_nanopub server {use_server}")
    headers = {'Content-Type': 'application/trig'}
    # Used by extended_nanopub-java: {'Content-Type': 'application/x-www-form-urlencoded'}
...
def verify_trusty(g: ConjunctiveGraph, source_uri: str, source_namespace: Namespace) -> bool:
    """Verify Trusty URI in a extended_nanopub Graph"""
    ...
    if expected_trusty != source_trusty:
        raise MalformedNanopubError(f"The Trusty artefact of the extended_nanopub {source_trusty} is not valid. It should be {expected_trusty}")
    else:
        return True


def verify_signature(g: ConjunctiveGraph, source_namespace: Namespace) -> bool:
    """Verify RSA signature in a extended_nanopub Graph"""
    # Get signature and public key from the triples
    np_sig = extract_enp_metadata(g)
