...
from extended_nanopub import ExtendedNanopub, NanopubConf
from extended_nanopub.profile import load_profile
from extended_nanopub._version import __version__
from extended_nanopub.definitions import DEFAULT_PROFILE_PATH, USER_CONFIG_DIR
from extended_nanopub.profile import Profile, ProfileError, generate_keyfiles
...
@cli.command(help='Get extended_nanopub library version')
def version():
    print(__version__)
...
def profile():
    try:
        ...
        print(" ℹ️  Use \033[1mnp setup\033[0m to setup your extended_nanopub profile locally with the interactive CLI")
...
def sign(
    ...
    private_key: Optional[Path] = typer.Option(
        None, "--private-key", "-k",
        help="Path to the RSA private key with which the extended_nanopub will be signed."
    ),
):
    if private_key:
        config = NanopubConf(
...
def publish(
    ...
    np = ExtendedNanopub(conf=config, rdf=filepath)
    np.publish()
...
def check(filepath: Path):
    ...
    try:
        np.is_valid
        print(f"\033[1m✅ Valid extended_nanopub\033[0m {np.source_uri}")
    except MalformedNanopubError as e:
        print(f"\033[1m❌ Invalid extended_nanopub\033[0m: {e}")


@cli.command(help='Interactive CLI to create a extended_nanopub user profile. '
                  'A local version of the profile will be stored in the user config dir '
                  '(by default $HOME/.extended_nanopub/). '
                  'The profile will also be published to the extended_nanopub servers.')
def setup(
    ...
    publish: Optional[bool] = typer.Option(
        ...
        help="If true, extended_nanopub will be published to extended_nanopub servers",
    ),
):
    """
    ...
    Args:
        ...
        publish: if True, profile will be published to extended_nanopub servers
        name: the name of the user
        keypair: a tuple containing the paths to the public and private RSA key to be used to sign
            nanopubs. If empty, new keys will be generated or the ones in the .extended_nanopub folder
            will be used.
    """
    print('⚙️ Setting up extended_nanopub profile...')
    ...
    if publish is None:
        prompt = ('📬️ Would you like to publish your profile to the extended_nanopub servers? '
    ...
    # Declare the user to extended_nanopub
    ...
    # Store profile extended_nanopub uri
