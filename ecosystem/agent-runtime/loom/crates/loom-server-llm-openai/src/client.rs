...
#[derive(Debug)]
struct OpenAIRequestError(LlmError);

impl RetryableError for OpenAIRequestError {
...
/// OpenAI API client.
///
/// Implements the LlmClient trait for interacting with OpenAI's Chat Completions API.
...
impl OpenAIClient {
	...
	fn build_request(&self, request: &LlmRequest, stream: bool) -> reqwest::RequestBuilder {
		let openai_request = OpenAIRequest::from_llm_request(request, stream);
	...
	async fn handle_error_response(&self, response: reqwest::Response) -> LlmError {
		...
		debug!(status = %status, "Received error response from OpenAI");
...
mod tests {
	...
	/// Tests that organization header is correctly set when configured.
	/// This is important for users who need to associate API calls
	/// with specific OpenAI organizations for billing or access control.
	#[test]
	fn test_config_with_organization() {
		let config = OpenAIConfig::new("test-api-key").with_organization("org-123");
