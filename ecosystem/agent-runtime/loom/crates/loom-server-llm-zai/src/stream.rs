...
//! Streaming SSE parser for Z.ai API responses (OpenAI-compatible format).

use crate::types::{ZaiError, ZaiStreamChunk};
use futures::Stream;
...
pin_project! {
		/// Stream adapter that parses Z.ai SSE events into LlmEvents.
		///
		/// # Purpose
		/// Converts raw Server-Sent Events from Z.ai streaming API into
		/// structured LlmEvent variants, handling delta accumulation and completion.
		/// Z.ai uses OpenAI-compatible SSE format.
		pub struct ZaiStream<S> {
				#[pin]
				inner: S,
				buffer: String,
...
impl<S, E> Stream for ZaiStream<S>
where
	...
	while let Some(line_end) = buffer.find('\n') {
		...
		if let Some(data) = line.strip_prefix("data: ") {
			...
			match serde_json::from_str::<ZaiStreamChunk>(data) {
				...
				Err(e) => {
					if let Ok(error_response) = serde_json::from_str::<ZaiError>(data) {
						warn!(
								error_type = ?error_response.error.error_type,
								message = %error_response.error.message,
								"Z.ai API error in stream"
						);
...
mod tests {
	...
	async fn test_stream_text_delta() {
		let chunks: Vec<Result<bytes::Bytes, std::io::Error>> = vec![
			Ok(bytes::Bytes::from(
				"data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"created\":1234,\"model\":\"glm-4.7\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"Hello\"},\"finish_reason\":null}]}\n\n",
			)),
			Ok(bytes::Bytes::from(
				"data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"created\":1234,\"model\":\"glm-4.7\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\" world\"},\"finish_reason\":null}]}\n\n",
			)),
			Ok(bytes::Bytes::from("data: [DONE]\n\n")),
		];

		let inner = futures::stream::iter(chunks);
		let mut stream = ZaiStream::new(inner);
	...
	/// This is important because the Z.ai API uses this marker to signal
	...
	/// with function arguments being streamed incrementally.
	#[tokio::test]
	async fn test_stream_tool_call_accumulation() {
		let chunks: Vec<Result<bytes::Bytes, std::io::Error>> = vec![
			Ok(bytes::Bytes::from(
				"data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"created\":1234,\"model\":\"glm-4.7\",\"choices\":[{\"index\":0,\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"call_123\",\"type\":\"function\",\"function\":{\"name\":\"get_weather\"}}]},\"finish_reason\":null}]}\n\n",
			)),
			Ok(bytes::Bytes::from(
				"data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"created\":1234,\"model\":\"glm-4.7\",\"choices\":[{\"index\":0,\"delta\":{\"tool_calls\":[{\"index\":0,\"function\":{\"arguments\":\"{\\\"loc\"}}]},\"finish_reason\":null}]}\n\n",
			)),
			Ok(bytes::Bytes::from(
				"data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"created\":1234,\"model\":\"glm-4.7\",\"choices\":[{\"index\":0,\"delta\":{\"tool_calls\":[{\"index\":0,\"function\":{\"arguments\":\"ation\\\":\\\"NYC\\\"}\"}}]},\"finish_reason\":null}]}\n\n",
			)),
			Ok(bytes::Bytes::from("data: [DONE]\n\n")),
		];
