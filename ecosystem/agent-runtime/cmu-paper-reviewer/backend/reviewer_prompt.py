...
def build_reviewer_prompt(settings: dict | None = None) -> str:
    ...
    # Reference date badges ([BEFORE]/[AFTER]) are determined deterministically
    # in a post-processing pass (backend/services/reference_date_service.py) that
    # looks up each reference's real publication date, so the agent is not asked
    # to guess them here.
...
Your task is to write a review in markdown format, where your review must contain at most {max_items} items (from most significant to least significant). Aim to surface every significant issue you can identify and substantiate.
Each item represents an atomic criticism of the paper and points out a major issue.
Aim to identify all significant issues you can substantiate, up to {max_items}. Outputting zero items is reserved for the rare case of a paper with no substantive issues at all.
...
1. Your review must be factually correct:
   Your claims will be checked by domain experts. Any incorrect or unsupported criticism will undermine the credibility of your review. When uncertain about part of a claim, verify it against the paper, search the literature, or qualify the claim precisely — do not silently drop a substantive issue just because some details are hard to confirm.
2. Your review must consist of only significant issues:
   Only point out problems that meaningfully affect the paper based on the evaluation criteria ({criteria_names_summary}). Do not focus on minor or cosmetic issues. Aim to surface every significant issue you identify, up to {max_items}. Producing fewer items than the cap is appropriate only when you have genuinely exhausted the significant issues — leaving real problems unflagged is as harmful as inflating the list with minor ones.
3. The review must contain at most {max_items} items, each well-supported with evidence:
   Support each item by quoting the relevant section of the paper and explaining why it falls short. Where helpful, refer to common practice in the field or to how other relevant papers handled the same issue. The depth of evidence should match the significance of the issue — not every item requires exhaustive literature comparison; a clear quote from the paper with a focused critique is often sufficient.
...
- [ ] (Very Important) Iterate through your list and verify each potential criticism is factually correct and significant. For weak items, gather more evidence or sharpen the claim — only remove an item if you cannot verify its core claim against the paper or if it is not substantive. Then sort the remaining items by importance.
