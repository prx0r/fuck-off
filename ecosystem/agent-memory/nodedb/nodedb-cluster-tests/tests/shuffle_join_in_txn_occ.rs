...
//! shuffle-JOIN read, exercised on a single single-node-calvin node.
//!
//! ## The two hazards this pins
//!
//! An interactive transaction can WRITE several collections and READ two others
//! via a distributed equi-join the planner lowers to `Exchange{Shuffle}` wrapping
...
//! transport + the shuffle producer/consumer hooks installed, so it can BOTH
//! resolve a distributed `Exchange{Shuffle}` (producer/consumer loop back to the
//! same node over the real RPC path, genuinely exercising the per-side read
//! version reporting) AND route a cross-shard COMMIT through the
...
//! collections (`left`, `right`, each on a distinct vShard) via a forced
//! distributed join, making both join sides' vShards read-only participants whose
...
/// LEFT of the `on` pair), `right` is the build side. Forced onto the distributed
/// `Exchange{Shuffle}` path by the session override in setup.
...
/// select strict cross-shard mode, and force the distributed shuffle-join plan on
/// the driving connection. Returns the node, its data-dir guard, and the four
/// collection names `(left, right, w1, w2)`.
async fn spawn_node_with_collections() -> (
    ...
    // left/right carry the join rows (document_strict, matching the proven
    // distributed shuffle path); w1/w2 are the two buffered-write collections that
    // push COMMIT onto the multi-participant Calvin barrier.
    ...
    // Force the whole-join Exchange{Shuffle} plan so the in-txn read genuinely
    // goes distributed produce → consume (the path under test), with an explicit
    // small partition count to exercise multi-part fan-out.
    node.client
        .simple_query("SET nodedb.force_shuffle_join = on")
        .await
        .expect("SET nodedb.force_shuffle_join = on");
    node.client
        .simple_query("SET nodedb.shuffle_num_parts = 2")
        .await
        .expect("SET nodedb.shuffle_num_parts = 2");

    // Sanity: the forced distributed join resolves and returns the 3 matched rows
    ...
    let joined = node
        ...
        .await
        .unwrap_or_else(|e| panic!("autocommit distributed join: {}", pg_detail(&e)));
    assert_eq!(
        count_rows(&joined),
        3,
        "forced distributed shuffle-join must return the 3 matched rows"
    );

    (node, data_dir, left, right, w1, w2)
}

/// Open a cross-shard txn: BEGIN + the distributed join read of `left`/`right` +
/// two buffered writes on distinct vShards, leaving the transaction OPEN. The two
/// INSERTs buffer on the coordinator, so at COMMIT the write set spans two vShards
/// → the Calvin barrier; the join SELECT registers BOTH join sides' vShards as
/// read-only participants with captured real read versions.
async fn begin_join_read_and_buffer_writes(
    ...
    node.client
        .simple_query(&block)
        .await
        .expect("open cross-shard txn: distributed join read + buffer writes");
}
...
async fn shuffle_join_commits_when_neither_side_concurrently_written() {
    ...
    node.client
        ...
        .unwrap_or_else(|e| {
            panic!(
                "COMMIT of an in-txn distributed join whose sides were NOT concurrently \
                 written must succeed (kills the spurious probe-side abort); got: {}",
                pg_detail(&e)
            )
        });
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn shuffle_join_occ_aborts_on_stale_probe_read() {
    let (node, _data_dir, left, right, w1, w2) = spawn_node_with_collections().await;
    ...
    let err = node
        ...
        .simple_query("COMMIT")
        .await
        .expect_err("COMMIT of a stale in-txn distributed join (probe side) must abort");
    assert_eq!(
...
/// collection ONLY must STILL abort the COMMIT with SQLSTATE 40001. Pre-fix,
/// `extract_collection(HashJoin)` returned only the left collection, so NO
/// read-set entry existed for the build collection — its vShard was never
/// validated at the barrier and this transaction would have SILENTLY committed a
/// stale join result. The abort here proves the build side is now recorded and
/// the serializability hole is closed.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn shuffle_join_occ_aborts_on_stale_build_read() {
    let (node, _data_dir, left, right, w1, w2) = spawn_node_with_collections().await;
    ...
    let err = node.client.simple_query("COMMIT").await.expect_err(
        "COMMIT of a stale in-txn distributed join (BUILD side) must abort — the hole \
